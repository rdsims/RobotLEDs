...
class APA102:
    def __init__(self, numLEDs, globalBrightness = 31, order='rgb'): # The number of LEDs in the Strip
        ...
        self.leds = [0,0,0,0] * self.numLEDs # Pixel buffer
        self.setGlobalBrightness(globalBrightness);	 # sets self.ledstart
    ...
    def setGlobalBrightness(self, brightness):
        self.globalBrightness = brightness;
        # LED startframe is three "1" bits, followed by 5 brightness bits
        self.ledstart = (brightness & 0b00011111) | 0b11100000 # Don't validate, just slash of extra bits
        # write brightness word as the 1st byte of every 4-byte color
        for k in range(0, self.numLEDs):
            self.leds[4*k] = self.ledstart


    def getGlobalBrightness(self):
        return self.globalBrightness
